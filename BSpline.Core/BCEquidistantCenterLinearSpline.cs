...
namespace BSpline.Core
{
    public sealed class BCEquidistantCenterLinearSpline
    {
        ...
        public double Evaluate(double x)
        {
            ...
            if (index <= -1)
            {
                return (x - _xmin - 0.5 * _dx) * (_data[1] - _data[0]) / _dx + _data[0];
            }

            if (_data.Count - 1 <= index)
            {
                return (x - _xmax + 0.5 * _dx) * (_data[_data.Count - 1] - _data[_data.Count - 2]) / _dx + _data[_data.Count - 1];
            }

            var help = ((x - _xmin) - index * _dx - _dx / 2.0) / _dx;
            return (1.0 - help) * _data[index] + help * _data[index + 1];
        }
